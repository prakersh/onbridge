...
CI (`.github/workflows/ci.yml`) runs all of it on every push and PR: typecheck, build, unit and integration tests, and both browser suites (under `xvfb-run` on Linux). Every suite uses ports 19876–19885, never the real range (see the invariant below).
...
- **Releases run on GitHub, never from a local machine.** The version is set in a PR (`./app.sh --bump`) and merged. `.github/workflows/release.yml`, started with `./app.sh --release` or Run workflow in the Actions tab, tags the `main` commit with `v<VERSION>` and then, in the same run, builds the GitHub release and publishes npm (trusted publishing is bound to that exact file name, so do not rename or split it). Its first job calls `ci.yml`, every automated suite, and nothing is tagged, released or published unless that passes; a new test belongs in `ci.yml`, never in a release-only step. A tag pushed by hand still triggers it too. The Chrome Web Store upload is separate, from the release's extension zip.
